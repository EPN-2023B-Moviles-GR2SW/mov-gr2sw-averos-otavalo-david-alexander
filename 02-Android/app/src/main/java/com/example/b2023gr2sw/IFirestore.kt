...
class IFirestore : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Order By
        // utiliza la funcion consultarConOrderBy, para coger la coleccion ciudades, se limpiara el arreglo y se notificara al adaptador
    ...
    fun consultarConOrderBy(
        ...
        val db = Firebase.firestore
        // tomamos la coleccion ciudades, lo limpiamos y notificamos al adaptador
        val citiesRefUnico = db.collection("cities")
        limpiarArreglo()
        adaptador.notifyDataSetChanged()
        citiesRefUnico
            //luego utilizamos la funcion orderBy para ordenar por una de las propiedades que tenemos de forma ascendente

            .orderBy("population", Query.Direction.ASCENDING)
            .get()
            //tenemos 2 listener por si pasa erres en el backend
            .addOnSuccessListener {
                ...
                //notificamos al adaptador que se ha modificado el arreglo en la vista
                adaptador.notifyDataSetChanged()
            }
            .addOnFailureListener {
                // Errores
